...
label daphne_pre_02:
    ...
    $ daphne.chibi.State("door", speed="go", appearance="a").Trans(d4, "blink").Trans("go center", "blink")
    pause.5
    $ screens.ShowD3("bld1")
...
label daphne_pre_07:
    ...
    if hermi.whoring<12 or (not end.IsEnding(const_ENDING_STRONG_GIRL) and hermi.liking <= 0):
        menu:
            "Стоит ли сейчас говорить с Гермионой о Дафне ?"
            "Конечно !":

                pass
            "Пожалуй, нет":

                jump chit_chat_hermione2

    $ hermi.State("door").Visibility("body+")
    $ music("music/Chipper Doodle v2.mp3")
    $ hero  (m, "Я хотел бы поговорить с вами, мисс, о некоторых девушках с вашего факультета.//")
    if not wtevent.IsExec( 1, "start1"):
    ...
    if hermi.whoring<12:
        ...
        $ hermi.liking-=10
        $ hero (m, "(Кажется, мы продвинулись недостаточно далеко)")
    else:
        ...
        if end.IsEnding(const_ENDING_STRONG_GIRL):
            ...
            $ hermi(tr("~body_83.png// Если она так и не даст, сэр? Ну это уже не мои проблемы, правда? И еще 10%% со всех ваших доходов."))
            ...
            if hermi.liking>0:
                ...
                menu:
                    "\"Будь по-твоему...\"":
                        ...
                        $ daphne.SetValue("lastVisitDay", day - 2)
                ...
                $ hermi.liking-=10
                $ hero (m,"(Ну вот. А ведь я чувствовал, что она не в том настроении. Кажется в каталоге было подходящее средство. )")
                $ hero (m, "(Призрак... Нет. Привидение... Нет, не то. Дух ? Духи !)")
    call music_block
    $ hermi.Visibility()
    jump hermione_goout
...
label daphne_pre_finish:
    ...
    $ music("Daphne Theme")
    $ daphne.chibi.State("door", speed="go").Trans(d4, "blink").Trans("go center", "blink")
    pause.5
    $ screens.ShowD3("bld1")
    ...
    $ daphne_pre_menu_text=".........."

    if wtevent._finishCount==0:
    ...
    $ music()
    if item==None:
        $ daphne("~55 01 1 pri// #Пообещал подарок, и где он? Скряга...")
        $ daphne.liking-=5
    $ screens.HideD3("bld1")
    $ daphne.Visibility(transition=d3).chibi.Trans("goout door").Hide(d3)
    $ renpy.play('sounds/door.mp3')
