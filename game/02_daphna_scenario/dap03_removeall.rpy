...
label dap_request_03_complete:
    ...
    if IsRunNumber(1):
        ...
        $ daphne.whoring += 1
        $ SetHearts(1,this.dap_request_03)
        ...
        $ daphne.whoring += 1
        $ SetHearts(2,this.dap_request_03)
        $ wtevent.Finalize("daphne_pre_menu")
    elif IsRunNumber(3):
        ...
        $ daphne.whoring += 1
        $ SetHearts(3,this.dap_request_03)
        $ wtevent.Finalize("daphne_pre_menu")
    elif IsRunNumber(4):
        ...
        $ hero("Вы это поняли!?")
        $ daphne("~77 01 2 pri// Он тяжело дышал, и смотрел мне в глаза...// Я... я не знаю что произошло...//~77 02 2 dis// Я не заметила как он достал эту штуку....// Я не понимала что происходит...//~77 w3 3 dis// Она была... была огромная...// Он сказал, что бы я обхватила его...//~26 w0 2 dis// Гхм... Я хотела уйти... Но он держал меня за руку, и тянул её туда...")
        $ hero("Вы поддались?")
        ...
        $ daphne.whoring += 1
        $ SetHearts(4,this.dap_request_03)
        $ wtevent.Finalize("daphne_pre_menu")
    elif IsRunNumber(5):
        $ hero("#(На мой взгляд, в этой сфере Дафна уже вполне преуспела)")
        call daphne_main_menu_requests
