...
label newsp_pre_snape_dialog:
    ...
    if nsp_pre_snape == 0 or nsp_pre_snape == 1:
        ...
        if snape_friendship > 40:
            ...
            $ hero("Подожди, а какая же тогда была первая ?")
...
label nsp_her_demo:
    ...
    $ renpy.say (m,tr("(По этой теме я уже сделал все, что мог.)"))
...
label nsp_her_demo2:
    ...
    $ renpy.say (m,tr("(Кажется, темы репортажей исчерпаны. Лучше заняться с ней чем-то другим.)"))
