label menu_reading_book:
    ...
    python:
        for e in this.List:
            if e.GetValue("block")==_block and e._status>=0:
                choose.AddItem("- Книга: "+e._caption+" - "+("{image=check_08.png}" if e.IsDone() else "{image=check_07.png}"),
    ...
    label menu_reading_book_2:
        ...
        $ renpy.say(none,tr(wtevent._caption) + ". " + tr(wtevent._description) + "\n")
        menu:
            "- Читать книгу -":
    ...
    label reading_book_done:
        ...
        if wtevent.Name=="book_07":
            ...
            if wtevent._block=="books_edu" and wtevent._conclusion!="":
                $ renpy.say(m,tr("Она дала мне новый навык: ") + tr (wtevent._conclusion))
            else:
                $ renpy.say(m,tr (wtevent._conclusion))
...
label desk:
    ...
    menu:
        ...
        "{color=#858585}- Делать бумажную работу -{/color}" if finished_report >= 6 and not got_paycheck:
            ...
            jump desk
        "{color=#858585}- Делать бумажную работу -{/color}" if got_paycheck:
        ...
        "- Тест -" if not day < 5:
            jump own_test
        "- Слушать музыку" if not day < 5:
            menu:
                "ru":
                    "Звуки поглощения кала"
                    g11"Слушаю это последний раз в жизни"
                    g11"..."
                    if daytime:
                        jump day_main_menu
                    else:
            ...
            show krk_sys
            if music_points <=8:
                $ renpy.play('sounds/Bladeesong1.mp3')

            m"..."
            if music_points == 1:

                g6"Он поёт о вампирах?"
            if music_points == 2:
            ...
            if music_points == 4:
                ...
                m"Почему здесь так много автотюна?"
            if music_points == 5:

                g7"Этот продакшн"
                g1"Бит сделан каким-то сверхчеловеком"
            if music_points == 6:

                g9"О этот голооос."
                g9"Как же офигенно настроен автотюн под его голос"
            if music_points == 7:

                g11"Ебать........"
            if music_points == 8:

                g11"Жизнь хуйня чувак"
                g11"..."
            if music_points == 9:

                $ renpy.play('sounds/igot_vampires.mp3')

                g12"DRAIN GANG"
                g12"I got vampires in my city"
            if music_points == 10:

                $ renpy.play('sounds/sick.mp3')

                g12"Still siiiick"
                g4"stil sick stil siiiick"
            if music_points == 11:

                $ renpy.play('sounds/my_magic.mp3')

                g4"YOU KNOW MY MAGIC IS STRONG"
                g12"YOU KNOW MY POWER IS endless"
                g12"You know that you are defenseless"
                g12"I'm gaining energy slowly"
                g12"I am so high that I'm holy"
                g12"I am so cold and I'm lonely"
                g12"If you love me then you show me"
                g12"This world shows me no affection"
                g12"I make the final connection"

            if music_points == 12:

                $ renpy.play('sounds/got_a_ghost.mp3')

                g12"Got a ghost in my basement,"
                g12"he come out when it's raining"
                g4"..."
                g4"Больше я не выдержу......."

            if music_points == 13:

                $ renpy.play('sounds/im_unreal.mp3')

                g12"Blue steel"
                g12"I'm unreal, I'm unreal, I'm unreal, I'm unreal"
                g11"I`m un...(плачет)...re.(рыдает)..al"
                g11"захлебается в слезах"
            if music_points == 14:

                $ renpy.play('sounds/frezee.mp3')

                g4"Above, disease"
                g12"JAPANEEEEEESE,never ease"
                g12"Walk on water,make it freeze"
            if music_points == 15:

                $ renpy.play('sounds/bleed.mp3')

                g12"I will make u bleed"
                g12"i will make u bleed"
            if music_points == 16:

                $ renpy.play('sounds/subaru.mp3')

                g12"Lost all essence, counting three blessings"
                g12"Wanna be legends, I live with depression"
                g12"C-section, razorblade lesson"
                g12"I know of three heavens, I want to see seven"
                g11"Ебаааааатть"
                g11"Я всё ещё жив?"
                g11"это невероятно"
            if music_points == 17:

                $ renpy.play('sounds/Bladee_skin.mp3')
                pause (36)
                g12"I don`t want to go back to that place again"
                pause (6)
                g11"Nothing will ever be the same again"
                g12"Neven even cared if its fake pretend"
                g12"Maybe you can show me how it take to end"
                g11"FOREVER"
                g12"Emptiness it came again"
                g12"When it`s from inside you know you can`t defend against it"
                g11"Writing but i can`t press send"
                g12"Kisses from the raindrops tell me wait again"
                g12"One last time."
                g11"Clean,clean,clean,cut the rope let me in"
                g11"Wings,wings wings,i grow wings i go in.(I GOIIIN)"
                g11"Scars,scars,scars i got scars on my arms(AaaAAAaa)"
                g11"Fall,fall,fall out love and appart(aaAAAaa)"
                g11"Skin,Skin,Skin let the skin touch the skin"
                g11"When,When,When,never was never been(Слёз становится все больше)"
                g11"I`m sinking its goes up to my chin(Дождь,непрекращая,все лил и лил,но ты не отличаешь его от своих слёз...)"
                g11"White,white,white,grey to black ever since."
                "С этого момента ты обрёл покой..."
                "Умерев в своей горести..."
                "утонув в дожде......."
                "Белый цвет - перешёл в серый"
                "Серый - в чёрный"
                "Небытие"
                g11"......."
            ...
            $ music_points +=1
            if music_points >18:
                g4"Чувак меня вытянули полумертвым оттудова"
                g4"Нехочу я это больше слушать"
...
label reading_book_xx:
    ...
    if daytime and not raining:
        ...
        play weather "sounds/day.mp3" fadeout 1.0 fadein 1.0


    if not daytime and not raining:
        play weather "sounds/night.mp3" fadeout 1.0 fadein 1.0
    ...
    if raining:
        $ renpy.say(none,tr(">Вы читаете книгу ") + tr(wtevent._caption) + tr(", слушая дождь, барабанящий по крыше вашей башни."))
    else:
        $ renpy.say(none,tr(">Вы читаете книгу ") + tr(wtevent._caption) + "...")
    ...
    if concentration>0:
        if concentration == 1:
            $ concentraton_check = renpy.random.randint(1, 6)
            if concentraton_check == 1:
                call concetrationg_reading
        if concentration == 2:
            $ concentraton_check = renpy.random.randint(1, 4)
            if concentraton_check == 1:
                call concetrationg_reading
        if concentration == 3:
            $ concentraton_check = renpy.random.randint(1, 2)
            if concentraton_check == 1:
                call concetrationg_reading
        if concentration == 4:
            call concetrationg_reading
    ...
    if s_reading_lvl>0:
        $ speed_dummies = Rand([60,30,10][s_reading_lvl-1]//turbo)
        if speed_dummies <= 10:
...
label chapter_check_book_xx:
    if (wtevent.IsDone() and wtevent.Name!="book_07") or (wtevent.Name=="book_07" and book_07_units == 20):
        if fire_in_fireplace:
        ...
        if wtevent.Name=="book_07":
            if complited_leena_already and complited_shea_already and complited_stevens_already and victoria >= 1 and shea >= 1 and leena >= 1:
                ...
                if not found_dahrs_ticket_once:
                    m "Хм...?"
                    m "Что это...? Закладка?"
                    $ the_gift = "03_hp/18_store/06.png"
                    show screen gift
                    with d3
                ...
                if not complited_stevens_already:
                ...
                if not complited_leena_already:
            ...
            if not dear_waifu_completed_once:
                $ dear_waifu_completed_once = True
                $ imagination +=1
        ...
        if wtevent._conclusion!="":
            if wtevent._block=="books_edu":
                $ renpy.say(m,tr("Новый навык: ") + tr(wtevent._conclusion))
            else:
                $ renpy.say(m,tr(wtevent._conclusion))
...
label paperwork:
    ...
    if daytime:
        play bg_sounds "sounds/day.mp3" fadeout 1.0 fadein 1.0
    else:
        play bg_sounds "sounds/night.mp3" fadeout 1.0 fadein 1.0
    ...
    call finished_working_chapter

    call report_chapters_check

    if daytime:
        pass
    else:
        if wather_generator >= 31 and wather_generator <= 40:
            call f_moon_bonus
        if wather_generator >= 51 and wather_generator <= 60:
            call f_moon_bonus
    ...
    if speedwriting == 1:
        $ speedwriting_check = renpy.random.randint(1, 6)
        if speedwriting_check == 1:
            call speedwriting_label
    if speedwriting == 2:
        $ speedwriting_check = renpy.random.randint(1, 4)
        if speedwriting_check == 1:
            call speedwriting_label
    if speedwriting == 3:
        $ speedwriting_check = renpy.random.randint(1, 2)
        if speedwriting_check == 1:
            call speedwriting_label
    if speedwriting == 4:
        call speedwriting_label
...
label nsp_articles_work:
    ...
    call nsp_finished_working_chapter

    call nsp_report_chapters_check
